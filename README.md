...
When we run pin level simulations in SystemC we sometimes want to use the 4-valued ```sc_logic``` value set in order to detect design bugs such as unconnected ports or ambiguous conditions that lead to X value propagation. While this can be beneficial for verification, it comes with the cost of slowing down the simulation. Other times, when are not so much concerned about connectivity bugs, we just want to run fast simulations with the 2-valued ```bool``` value set to find other types of bugs.

It would be very conventient to have a simple way of switching between the two value sets automatically without having to change the entire source code by hand. 

## Use define switches
We can use define switches like ```SC_USE_BOOL_TYPES``` and ```SC_USE_LOGIC_TYPES``` for each value set that we want to use. Defining one switch or the other at compilation will force the chosen value set over the entire project provided that we respect some coding rules and guidelines that will be described below. Let's also decide that by default we are going to use SC_USE_BOOL_TYPES if none of these switches are defined. The source code for this default option would look like this:
```cpp
...
* __bool__ uses ```false``` for 0 and ```true``` for 1 (it's also valid to use 0 for ```false``` and non-zero for ```true```)
* __sc_logic__ uses ```SC_LOGIC_0``` for 0 and ```SC_LOGIC_1``` for 1
* __sc_logic__ has two extra values ```SC_LOGIC_X``` and ```SC_LOGIC_Z```
* ```SC_LOGIC_0``` and ```SC_LOGIC_1``` are constant class instances not enum values nor integer constants so they can't be cast or assigned to integers.
* __SOLUTION:__
  ...
  * Code that uses the ```SC_LOGIC_X/SC_LOGIC_Z``` values must be enclosed by ```#ifdef SC_USE_LOGIC_TYPES ... #endif``` to avoid compilation or runtime errors
...
* __WARNING:__ do not use the ```~``` operator on ```bool``` variables for logical negation, it will return wrong results.
  * In C/C++ ```~``` is used for bitwise negation
  * In C++ the ```bool``` is internally represented as an integer (bit size depends on the implementation but we can assume it's 8 bits which is the smallest unit of addressable memory in most cases)
  * Applying ```~``` to ```false```(0x00) will return 0xFF which is non-zero which in the ```bool``` world is equal to ```true``` so there's no problem here
  * Applying ```~``` to ```true```(0x01) will return 0xFE which is non-zero which in the ```bool``` world is equal to ```true``` which is wrong because we want the logical negation of ```true``` to be ```false```  
* __SOLUTION:__
...
* both __sc_bv__ and __sc_lv__ variables can be assigned the following types of literals:
  * integer literals: e.g. 85 (decimal), 0213 (octal), 0x4b (hexadecimal), 30 (int)m, 30u (unsigned int), 30l (long), 30ul (unsigned long), etc.
  * string literals representing integer values: e.g.  "0b10100",  "0d478", "0xFF", etc. (See the SystemC LRM for more details on formatting)
...
* both ```bool``` and ```sc_logic``` variables can be assigned the following literals:
  ...
  * character literals e.g. 'a' 'b' 'c'
    * ```bool``` will see '\0' as false and everythin else as true
    * ```sc_logic``` will only accept chars with ASCII codes that are less than 127 otherwise an error message will the printed and the conversion will return a value of SC_LOGIC_X
      * for ASCII codes 0, 1, 2 and 3 the conversion will return values SC_LOGIC_0, SC_LOGIC_1, SC_LOGIC_Z, SC_LOGIC_X
      * for ASCII codes of characters '0', '1', 'z' and 'Z', 'x' and 'X' the conversion will return value SC_LOGIC_0, SC_LOGIC_1, SC_LOGIC_Z, SC_LOGIC_X
...
* __SOLUTION:__
  ...
  * For performance purposes it's recommended to assign integer literals to ```sc_vec_type``` variables
  * For readability purposes it's recommended to assign string or character literals to ```sc_vec_type``` variables
...
```
### Arithmetic operations are not possible for sc_logic, sv_bv and sc_lv and the workarounds can yield different effects
...
* In order to do integer arithmetic computations with variables of these types, they need to be copied to ```sc_int``` or ```sc_uint``` or other integer variables or data structures, do the computations and then convert the results back to types ```sc_logic```, ```sc_bv``` or ```sc_lv```
...
* If ```sc_logic``` or ```sc_lv``` variables containing X/Z values are assigned to ```sc_int``` or ```sc_uint``` variables then the X/Z bits are translated to '1', e.g. "XXZZ01" translates to "111101". This means that X/Z values get lost when doing arithmetic operations with ```sc_logic``` and ```sc_lv``` variables.
* __SOLUTION:__
  * Add a function ```bool has_xz(sc_vec_type)``` that checks if an ```sc_vec_type``` variable contains X/Z values
...
* According to the "SystemC Synthesis Subset v1.4.7" the inpot port type ```sc_in<sc_logic>``` is not synthesizable so make sure that you define SC_USE_BOOL_TYPES to make sure that your code is synthesizable. Another solution is to declare all input ports as sc_in<bool> and propagate their values to internal sc_logic variables.
* For performance purposes it's recommended to assign integer literals to ```sc_vec_type``` variables
* For readability purposes it's recommended to assign string or character literals to ```sc_vec_type``` variables
...
## SR Latch example
Let's apply this solution to a simple example: an SR latch. SR latches are level sensitive storage elements with two inputs S(Set) and R(Reset) and two outputs Q and NQ. It stores a value of 1 (Q=1, NQ=0) when the S input toggles from low to high and R is low. It stores a value of 0 (Q=0, NQ=1) when the R input toggles from low to high and S is low. When both S and R are low the latch holds the current stored value. When S and R are both high this is an illegal combination and the output is X. See below the code of an SR latch model and a testbench module that applies some stimuli to its inputs:
```cpp
...
And this is how you run the same scenario with different value sets.
